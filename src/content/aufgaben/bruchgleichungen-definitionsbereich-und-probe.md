---
titel: "Bruchgleichungen – Definitionsbereich zuerst, Probe zuletzt"
thema: "Bruchgleichungen"
klassenstufe: ["9", "10"]
schwierigkeit: vertieft
...
tags: ["bruchgleichungen", "definitionsbereich", "scheinloesung", "hauptnenner", "probe"]
datum: 2026-08-28
entwurf: false
---
...
**Vor dem Rechnen:** Bestimmen Sie den **Definitionsbereich**. Welche Werte machen einen Nenner null? Diese Werte sind verboten – für die ganze Aufgabe.

**Nach dem Rechnen:** Machen Sie die **Probe**. Beim Multiplizieren mit dem Hauptnenner können Lösungen entstehen, die die ursprüngliche Gleichung gar nicht erfüllen. Man nennt sie *Scheinlösungen*.
...
| 3 | `3/(x+5) = 6` | x = −5 |
| 4 | `3/(2x−6) = 1` | x = 3 |
| 5 | `1/x + 1/(x−1) = 3` | x = 0 und x = 1 |
| 6 | `1/(x²−4) = 5` | x = 2 und x = −2 |
...
| Nr. | Gleichung | Definitionsbereich | Lösung |
|----:|:----------|:-------------------|:-------|
| 13 | `1/x = 2/(x+3)` | x ≠ 0, x ≠ −3 | x = 3 |
| 14 | `2/x = 3/(x+1)` | x ≠ 0, x ≠ −1 | x = 2 |
| 15 | `1/(x−1) = 2/(x+1)` | x ≠ 1, x ≠ −1 | x = 3 |
| 16 | `1/x + 1/(2x) = 3` | x ≠ 0 | x = 0,5 |
| 17 | `1/(x−2) = 3/(x+2)` | x ≠ 2, x ≠ −2 | x = 4 |
| 18 | `4/x − 1/x = 3` | x ≠ 0 | x = 1 |
...
|----:|:----------|:------------|:------------|
| 19 | `x/(x−2) = 2/(x−2)` | x = 2 | keine Lösung (x = 2 verboten) |
| 20 | `1/(x−3) = 0` | – | keine Lösung: ein Bruch mit Zähler 1 wird nie null |
| 21 | `x/(x−1) − 1/(x−1) = 0` | x = 1 | keine Lösung (x = 1 verboten) |
| 22 | `(x²−9)/(x−3) = 6` | x = 3 | keine Lösung (x = 3 verboten) |
| 23 | `1/(x+2) = 1/(x+2)` | jedes x | alle x außer x = −2 |
| 24 | `3/(x−4) + 12/(x−4) = 5` | x = 7 | x = 7 ist gültig |
...
|----:|:--------|:------------------|
| 25 | Eine Zahl und ihr Kehrwert ergeben zusammen 2,5. Welche Zahl? | `x + 1/x = 2,5` → `x = 2` oder `x = 0,5`. |
| 26 | Zwei Rohre füllen ein Becken zusammen in 6 Stunden. Rohr A allein braucht 10 Stunden. Wie lange braucht B allein? | `1/10 + 1/x = 1/6` → x = 15 Stunden. |
| 27 | Ein Bruch hat den Zähler 3. Vergrößert man den Nenner um 2, halbiert sich der Wert. Wie heißt der Bruch? | `3/n : 2 = 3/(n+2)` → n = 2, also `3/2`. |
| 28 | Bei welcher Zahl ist der Kehrwert viermal so groß wie die Zahl selbst? | `1/x = 4x` → `x = 0,5` oder `x = −0,5`. |
| 29 | Zwei Arbeiter brauchen zusammen 4 Stunden. Einer allein braucht 6 Stunden. Wie lange braucht der andere? | `1/6 + 1/x = 1/4` → x = 12 Stunden. |
| 30 | Der Kehrwert einer Zahl, um 1 vergrößert, ergibt 3. Welche Zahl? | `1/x + 1 = 3` → `x = 0,5`. |

## Reflexionsfragen

1. Warum ist das Multiplizieren mit dem Hauptnenner keine Äquivalenzumformung, solange der Nenner null werden kann?
2. In Nr. 20 gibt es keine Lösung. Begründen Sie das ohne Rechnung.
...
5. Aufgabe 26 und 29 haben dieselbe Struktur. Formulieren Sie den gemeinsamen Ansatz in einem Satz.
...
- *„Über Kreuz multiplizieren" überall.* Nr. 16 und 18. Gegenmittel: Die Regel gilt für die Form `a/b = c/d` – erst prüfen, ob sie vorliegt.
...
- [Übungsgenerator Bruchgleichungen](/uebung/bruchgleichungen)
- [Diagnostische Fragen zum Definitionsbereich](/quizzes)
- [Aufgabenfolge zu Bruchtermen](/aufgaben/bruchterme-kuerzen-und-addieren) – die Vorstufe ohne Gleichheitszeichen
