...
- $log_{a}(x \cdot y) = log_{a}(x) + log_{a}(y)$ (Produkt → Summe)
- $log_a(x / y) = log_a(x) - log_a(y)$ (Quotient → Differenz)
- $log_{a}(x^{n}) = n \cdot log_{a}(x)$ (Potenz → Faktor)

Damit kann man exponentielle Gleichungen wie $a^{t} = b$ lösen: $t = log_{a}(b)$ oder $t = log(b) / log(a)$.
...
| 5 | $2^{3} = 8$ | $log_{2}(8) = 3$ |
| 6 | $2^{4} = 16$ | $log_{2}(16) = 4$ |
| 7 | $5^{2} = 25$ | $log_{5}(25) = 2$ |
| 8 | $7^{0} = 1$ | $log_{7}(1) = 0$ |
| 9 | $(½)^{2} = ¼$ | $log_{1/2}(1/4) = 2$ |
...
| 15 | $log_2(1/4)$ | −2 |
| 16 | $log_2(2)$ | 1 |
| 17 | $log_4(2)$ | $1/2 (denn 4^{1/2} = \sqrt 4 = 2)$ |
| 18 | $log_3(\sqrt 3)$ | $1/2$ |
...
|----:|:----------|:-----------|:-------|
| 26 | $2^{t} = 64$ | direkt: $t = log_{2}(64)$ | $t = 6$ |
| 27 | $2^{t} = 1000$ | $$t = log_{2}(1000) = log(1000)/log(2)$$ | $`t \approx 9{,}966`$ |
| 28 | $10^{t} = 50$ | $t = log(50)$ | $`t \approx 1{,}699`$ |
| 29 | $1{,}05^{t} = 2$ (Verdopplung bei 5 % p.a.) | $$t = log(2)/log(1{,}05)$$ | $`t \approx 14{,}21`$ |
| 30 | $0{,}9^{t} = 0{,}5$ (Halbwertszeit bei 10 % Abnahme) | $$t = log(0{,}5)/log(0{,}9)$$ | $`t \approx 6{,}58`$ |
| 31 | $e^{t} = 10$ | $t = ln(10)$ | $`t \approx 2{,}303`$ |
| 32 | $5 \cdot 2^{t} = 80$ | erst durch 5: $2^{t} = 16$, dann $t = log_{2}(16)$ | $t = 4$ |
...
1. Aufgabe 3 zeigt: $log(1) = 0$. Warum gilt das für *jede* Basis (also auch $log_{2}(1) = 0$, $log_{5}(1) = 0$, ...)?
